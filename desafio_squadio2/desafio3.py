def verificar_forca_senha(senha):
    ...
    for sequencia in senha:
        if sequencia.islower():
            tem_letra_minuscula = True
        if sequencia.isupper():
            tem_letra_maiuscula = True
    if not tem_letra_maiuscula | tem_letra_minuscula:
        return "Sua senha deve conter letras maiúsculas e minúsculas. Tente uma senha mais complexa."
    ...
    # Verificando se a senha contém caracteres especiais e números
    especiais = ['@', '#', '$', '%', '!', '*', '&']
    for sequencia in senha:
        if sequencia.isnumeric():
            tem_numero = True
        if sequencia in especiais:
            tem_caractere_especial = True
    if not tem_numero & tem_caractere_especial:
        return "Sua senha nao atende aos requisitos de seguranca."

    # TODO: Verificar o comprimento mínimo e critérios de validação

    if tem_letra_maiuscula & tem_letra_minuscula & tem_caractere_especial & tem_numero:
